...
void imprimir(){
	...
	if(!verificarVazia()) {
		...
		while(aux != NULL) {
			...
			printf("\nIdentificador %d \t Setor %s \t Peso %.2f", aux->dado.identificador, aux->dado.setor, aux->dado.peso);
	...
	do {
		...
		switch(opcao) {
			case 1:
				...
				break;
			case 2:
				printf("\nDigite o numero identificador da caixa: ");
				scanf("%d", &temp.identificador);
                fflush(stdin);
				printf("\nDigite o setor da caixa: ");
				scanf("%[^\n]s", temp.setor);
				fflush(stdin);
			...
			case 3:
				temp = pop();
				printf("Identificador da caixa removida: %d", temp.identificador);
				break;
			case 4:
				imprimir();
				break;
