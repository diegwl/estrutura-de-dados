...
public class ArvoreCodigo {
	...
	private int pegaProfundidadeDoNoAteRaiz(No noReferencia, No noBuscado) {
		if (noReferencia == null) {
			...
			if (noBuscado.getCodigoContato() < noReferencia.getCodigoContato()) {
	...
	private No procuraNo(int valor, No noReferencia) {
		if (noReferencia == null || valor == noReferencia.getCodigoContato()) {
			return noReferencia;
		} else if (valor <= noReferencia.getCodigoContato()) {
	...
	public void removeNoPorValor(int valorARemover) {
		...
		No atual = raiz;
		while (atual != null) {
			if (valorARemover == atual.getCodigoContato()) {
				if (atual.ehFolha()) {
					...
					removeNoPorValor(valorSucessor.getCodigo());
					atual.setValor(valorSucessor);
				}
				return;
			} else if (valorARemover < atual.getCodigoContato()) {
				pai = atual;
				atual = atual.getEsquerdo();
			} else {
				pai = atual;
	...
	public int pegarMaiorValor() {
		...
		return percorrerDireita(raiz).getCodigoContato();
	}
